...
However, cards built on Intersil Prism2 hardware received firmware updates to support WPA (TKIP) via a host-side supplicant, which also permits the driver to support WPA2 (CCMP). In both cases encryption is handled by the host CPU rather than by the card itself.
